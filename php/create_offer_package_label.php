<?php
$filePath = 'uploads/';
$filename = 'pakbon-' . $order['numb'];
$file = $filename;

class CreateOfferPackageLabel extends FPDF
{
// Page header
    function Header()
    {
...
$pdf = new CreateOfferPackageLabel();
...
// Line break
    $pdf->Ln(5);
// Invoice Nr
    $pdf->Cell(80, 15, 'Pakbon: #' . $order['numb'], 0, 0, 'C');
...
// Set Font Normal
    ...
    foreach ($offerPackageLabel as $resOffer) {
        $getProductInfo = $conn->prepare("SELECT productNumber, productName, brand FROM product WHERE productID = ?");
        $getProductInfo->execute([$resOffer['productID']]);
        $resultOffer = $getProductInfo->fetch();

        $pdf->SetX(20);
        $pdf->Cell(20, 8, $resOffer['amount'], 'T,B', 0, 'L');
        $pdf->Cell(50, 8, $resultOffer['productNumber'], 'T,B', 0, 'L');
        $pdf->Cell(100, 8, $resultOffer['brand'] . ' ' . $resultOffer['productName'], 'T,B', 0, 'L');
        $pdf->Ln(8);
    }
