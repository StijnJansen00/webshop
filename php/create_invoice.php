...
// Set Info further of the side
    $pdf->SetX(20);
// Company Info
    $pdf->Cell(15, 5, $companyName, 0, 2, 'L');
    $pdf->Cell(15, 5, $name . ' ' . $surname, 0, 2, 'L');
    $pdf->Cell(15, 5, $phone, 0, 2, 'L');
    $pdf->Cell(15, 5, $street . $number, 0, 2, 'L');
    $pdf->Cell(15, 5, $zipcode . '  ' . $city, 0, 2, 'L');
    $pdf->Cell(15, 5, 'KvK: ' . $kvk, 0, 2, 'L');
    if (!empty($reference)) {
        $pdf->Cell(15, 5, 'Referentie: ' . $reference, 0, 2, 'L');
    }
...
// Line break
    $pdf->Ln(5);
// Invoice Nr
    $pdf->Cell(80, 15, 'Factuur: #' . $invoiceNr, 0, 0, 'C');
...
// Set Info further of the side
    $pdf->SetX(20);
// Dear Client
    $pdf->Cell(15, 5, 'Beste ' . $name . ' ' . $surname . ',', 0, 0, 'L');
// Line break
    $pdf->Ln(8);
// Set Info further of the side
    $pdf->SetX(20);
...
// Empty row
    ...
    if ($couponValue > 0) {
        if ($eur === true) {
            $priceExcl = $priceExcl - $couponValue;
            $c = $couponValue . '.00';
            $pdf->Cell(60, 6, 'Coupon: ' . $coupon . ' ' . EURO, 0, 0, 'R');
        } else {
            $priceExcl = $priceExcl - (($priceExcl / 100) * $couponValue);
            $c = $couponValue . '%';
            $pdf->Cell(60, 6, 'Coupon: ' . $coupon, 0, 0, 'R');
        }
        $pdf->Cell(20, 6, $c, 0, 1, 'R');
// Line break
        $pdf->Ln(1);
// Empty row
        $pdf->Cell(90, 6);
